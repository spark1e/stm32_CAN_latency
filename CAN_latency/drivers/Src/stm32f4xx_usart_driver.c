...
void USART_PeriClockControl(USART_RegDef_t *pUSARTx, uint8_t EnorDi){
	if(EnorDi == ENABLE){
		...
		else if(pUSARTx == UART7)  UART7_PCLK_EN();
		else if(pUSARTx == UART8)  UART8_PCLK_EN();
	}
	else{
		...
		else if(pUSARTx == UART7)  UART7_PCLK_DI();
		else if(pUSARTx == UART8)  UART8_PCLK_DI();
	}
}
...
void USART_DeInit(USART_RegDef_t *pUSARTx){
	...
	else if(pUSARTx == UART7)  UART7_REG_RESET();
	else if(pUSARTx == UART8)  UART8_REG_RESET();
}
...
//			pUSARTHandle->pUSARTx->USART_DR = (*pdata & (uint16_t)0x01FF);	//mask with 0000 0001 1111 1111, DR is 32 bits
//
...
void USART_SendData(USART_Handle_t *pUSARTHandle, uint8_t *pTxBuffer, uint32_t Len)
{
   ...
   //Loop over until "Len" number of bytes are transferred
	for(uint32_t i = 0 ; i < Len; i++)
	{
		...
		//DATA IS CORRUPTED WITHOUT WAITING
...
void USART_ReceiveData(USART_Handle_t *pUSARTHandle, uint8_t *pRxBuffer, uint32_t Len)
{
   //Loop over until "Len" number of bytes are transferred
	for(uint32_t i = 0 ; i < Len; i++)
	{
		...
		while(!(pUSARTHandle->pUSARTx->USART_SR & USART_GetFlagStatus(pUSARTHandle->pUSARTx, USART_FLAG_TXNE)));
